...
function sanitizeIsolirGuideSettings(payload = {}, current = {}) {
  const text = (name, fallback = '', max = 180) => {
    const value = Object.prototype.hasOwnProperty.call(payload, name) ? payload[name] : current[name];
    return String(value || fallback).trim().slice(0, max);
  };
  return {
    sourceSubnet: text('sourceSubnet', '172.30.0.0/16', 64),
    billingServerIp: text('billingServerIp', '', 64),
    isolirProtocol: ['http', 'https'].includes(String(payload.isolirProtocol || current.isolirProtocol)) ? String(payload.isolirProtocol || current.isolirProtocol) : 'http',
    isolirUrl: text('isolirUrl', '', 240),
    proxyPort: text('proxyPort', '8080', 8),
    wanList: text('wanList', 'WAN', 80),
    wanInterface: text('wanInterface', '', 80),
    comment: text('comment', 'Generated by Billing', 120)
  };
}
...
async function handleApi(req, res, url) {
  ...
  if (method === 'PUT' && pathname === '/api/settings') {
    ...
    const { data } = await mutate((store) => {
      ...
      if (payload.isolirGuide && typeof payload.isolirGuide === 'object') {
        store.settings.isolirGuide = sanitizeIsolirGuideSettings(payload.isolirGuide, store.settings.isolirGuide || {});
      }
