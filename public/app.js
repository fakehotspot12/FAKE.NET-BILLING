...
function buildIsolirRouterOsScript(options = {}) {
  ...
  const cleanCommentPattern = String(comment || 'Generated by Billing').replace(/[\\"]/g, '');
  const lines = [
    ':local sourceSubnet ' + routerOsQuoted(sourceSubnet),
    ':local billingServerIp ' + routerOsQuoted(billingServerIp),
    ':local isolirHost ' + routerOsQuoted(isolirHost),
    ':local proxyPort ' + routerOsQuoted(proxyPort),
    ':local wanList ' + routerOsQuoted(wanList),
    ':local wanInterface ' + routerOsQuoted(wanInterface),
    ':local addressList ' + routerOsQuoted(addressList),
    ':local comment ' + routerOsQuoted(comment),
    ':local proxyComment ' + routerOsQuoted(proxyComment),
    '',
    '/ip proxy access remove [find comment~' + routerOsQuoted(cleanCommentPattern) + ']',
    '/ip firewall nat remove [find comment~' + routerOsQuoted(cleanCommentPattern) + ']',
    '/ip firewall filter remove [find comment~' + routerOsQuoted(cleanCommentPattern) + ']',
    '/ip firewall address-list remove [find comment~' + routerOsQuoted(cleanCommentPattern) + ']',
    '/ip firewall address-list remove [find list=$addressList]',
    '',
    ':if ([:len [/interface list find where name=$wanList]] = 0) do={ /interface list add name=$wanList comment=$comment }',
    ':if ([:len $wanInterface] > 0) do={ :if ([:len [/interface find where name=$wanInterface]] = 0) do={ :put ("WARNING: interface WAN " . $wanInterface . " tidak ditemukan, cek nama interface") } else={ :if ([:len [/interface list member find where list=$wanList interface=$wanInterface]] = 0) do={ /interface list member add list=$wanList interface=$wanInterface comment=$comment } } }',
    ':if ([:len [/interface list member find where list=$wanList]] = 0) do={ :put ("WARNING: interface-list " . $wanList . " belum punya member. Isi field Interface WAN atau tambahkan manual /interface list member.") }',
    '',
    '/ip proxy set enabled=yes port=$proxyPort anonymous=no cache-administrator=admin@example.net',
    '/ip proxy access add dst-port=$proxyPort action=allow comment=$proxyComment',
    '/ip proxy access add dst-host=$isolirHost action=allow comment=$proxyComment',
    '/ip proxy access add src-address=$sourceSubnet dst-host=!$isolirHost action=redirect action-data=$isolirHost comment=$proxyComment',
    '',
    '/ip firewall nat add chain=dstnat action=redirect to-ports=53 protocol=udp src-address=$sourceSubnet dst-address=!$billingServerIp dst-port=53,5353 place-before=0 comment=($comment . " - redirect DNS isolir")',
    '/ip firewall nat add chain=dstnat action=redirect to-ports=53 protocol=tcp src-address=$sourceSubnet dst-address=!$billingServerIp dst-port=53,5353 place-before=0 comment=($comment . " - redirect DNS isolir")',
    '/ip firewall nat add chain=dstnat action=redirect to-ports=$proxyPort protocol=tcp src-address=$sourceSubnet dst-address=!$billingServerIp dst-port=80 place-before=0 comment=$proxyComment',
    '/ip firewall nat add chain=dstnat action=accept protocol=tcp src-address=$sourceSubnet dst-address=$billingServerIp place-before=0 comment=($comment . " - bypass billing isolir")',
    '',
    '/ip firewall filter add chain=forward action=drop protocol=udp src-address=$sourceSubnet dst-address=!$billingServerIp dst-port=!53,5353 place-before=0 comment=$proxyComment',
    '/ip firewall filter add chain=forward action=drop protocol=tcp src-address=$sourceSubnet dst-address=!$billingServerIp place-before=0 comment=$proxyComment',
    '/ip firewall filter add chain=forward action=accept src-address=$sourceSubnet dst-address=$billingServerIp place-before=0 comment=($comment . " - allow billing isolir")',
    '/ip firewall filter add chain=input action=drop protocol=tcp in-interface-list=$wanList dst-port=$proxyPort place-before=0 comment=($comment . " - block webproxy public")',
    '/ip firewall filter add chain=input action=accept protocol=tcp src-address=$sourceSubnet dst-port=$proxyPort place-before=0 comment=($comment . " - allow webproxy local")',
    ...(httpsMode ? [
      '',
      '/ip firewall address-list add list=$addressList address=$isolirHost comment=($proxyComment . " HTTPS")',
      '/ip firewall filter add chain=forward action=accept protocol=tcp src-address=$sourceSubnet dst-address-list=$addressList dst-port=443 place-before=0 comment=($proxyComment . " HTTPS")'
    ] : []),
    '',
  ...
  return lines.filter(Boolean).map((line) => line.endsWith(';') ? line : `${line};`).join('\n');
}
