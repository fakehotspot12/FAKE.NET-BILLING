...
function routerOsValue(value = '') {
  const text = String(value || '').trim();
  return /^[!A-Za-z0-9_.:/-]+$/.test(text) ? text : routerOsQuoted(text);
}

function routerOsCommentMatch(value = '') {
  return routerOsQuoted(String(value || 'Generated by Billing').replace(/[\\"]/g, ''));
}
...
function buildIsolirRouterOsScript(options = {}) {
  ...
  const wanList = String(options.wanList || 'WAN').trim();
  const wanInterface = String(options.wanInterface || '').trim();
  const comment = String(options.comment || 'Generated by Billing').trim();
  const proxyComment = `${comment} - Isolir WebProxy`;
  const cleanCommentPattern = routerOsCommentMatch(comment);
  const src = routerOsValue(sourceSubnet);
  const billingIp = routerOsValue(billingServerIp);
  const negBillingIp = routerOsValue(`!${billingServerIp}`);
  const host = routerOsQuoted(isolirHost);
  const proxy = routerOsValue(proxyPort);
  const wan = routerOsQuoted(wanList);
  const iface = routerOsQuoted(wanInterface);
  const commentText = routerOsQuoted(comment);
  const proxyCommentText = routerOsQuoted(proxyComment);
  const lines = [
    '# Script redirect isolir FAKE.NET Billing - RouterOS v6/v7',
    '# Paste dari awal sampai akhir di terminal MikroTik.',
    '# Wajib: subnet isolir harus sama dengan pool/profile EXPIRED di billing.',
    '',
    `/ip proxy access remove [find where comment~${cleanCommentPattern}]`,
    `/ip firewall nat remove [find where comment~${cleanCommentPattern}]`,
    `/ip firewall filter remove [find where comment~${cleanCommentPattern}]`,
    `/ip firewall address-list remove [find where comment~${cleanCommentPattern}]`,
    `/ip firewall address-list remove [find where list=${routerOsQuoted(addressList)}]`,
    '',
    `:if ([:len [/interface list find where name=${wan}]] = 0) do={ /interface list add name=${wan} comment=${commentText} }`,
    ...(wanInterface ? [
      `:if ([:len [/interface find where name=${iface}]] > 0) do={ :if ([:len [/interface list member find where list=${wan} interface=${iface}]] = 0) do={ /interface list member add list=${wan} interface=${iface} comment=${commentText} } } else={ :put "WARNING: interface WAN tidak ditemukan, cek nama interface" }`
    ] : []),
    `:if ([:len [/interface list member find where list=${wan}]] = 0) do={ :put "WARNING: interface-list WAN belum punya member. Isi Interface WAN atau tambahkan manual /interface list member." }`,
    '',
    `/ip proxy set enabled=yes port=${proxy} anonymous=no cache-administrator=admin@example.net`,
    `/ip proxy access add dst-host=${host} action=allow comment=${proxyCommentText}`,
    `/ip proxy access add src-address=${src} action=redirect action-data=${host} comment=${proxyCommentText}`,
    '',
    `/ip firewall nat add chain=dstnat action=redirect to-ports=${proxy} protocol=tcp src-address=${src} dst-address=${negBillingIp} dst-port=80 comment=${proxyCommentText}`,
    `/ip firewall nat add chain=dstnat action=redirect to-ports=53 protocol=tcp src-address=${src} dst-address=${negBillingIp} dst-port=53,5353 comment=${routerOsQuoted(`${comment} - redirect DNS isolir`)}`,
    `/ip firewall nat add chain=dstnat action=redirect to-ports=53 protocol=udp src-address=${src} dst-address=${negBillingIp} dst-port=53,5353 comment=${routerOsQuoted(`${comment} - redirect DNS isolir`)}`,
    `/ip firewall nat add chain=dstnat action=accept src-address=${src} dst-address=${billingIp} comment=${routerOsQuoted(`${comment} - bypass billing isolir`)}`,
    `/ip firewall nat move [find where comment=${proxyCommentText}] destination=0`,
    `/ip firewall nat move [find where comment=${routerOsQuoted(`${comment} - redirect DNS isolir`)}] destination=0`,
    `/ip firewall nat move [find where comment=${routerOsQuoted(`${comment} - bypass billing isolir`)}] destination=0`,
    '',
    `/ip firewall filter add chain=forward action=drop protocol=udp src-address=${src} dst-address=${negBillingIp} comment=${proxyCommentText}`,
    `/ip firewall filter add chain=forward action=drop protocol=tcp src-address=${src} dst-address=${negBillingIp} comment=${proxyCommentText}`,
    `/ip firewall filter add chain=forward action=accept src-address=${src} dst-address=${billingIp} comment=${routerOsQuoted(`${comment} - allow billing isolir`)}`,
    `/ip firewall filter add chain=input action=drop protocol=tcp in-interface-list=${wan} dst-port=${proxy} comment=${routerOsQuoted(`${comment} - block webproxy public`)}`,
    `/ip firewall filter add chain=input action=accept protocol=tcp src-address=${src} dst-port=${proxy} comment=${routerOsQuoted(`${comment} - allow webproxy local`)}`,
    ...(httpsMode ? [
      '',
      `/ip firewall address-list add list=${routerOsQuoted(addressList)} address=${host} comment=${routerOsQuoted(`${proxyComment} HTTPS`)}`,
      `/ip firewall filter add chain=forward action=accept protocol=tcp src-address=${src} dst-address-list=${routerOsQuoted(addressList)} dst-port=443 comment=${routerOsQuoted(`${proxyComment} HTTPS`)}`
    ] : []),
    `/ip firewall filter move [find where comment=${routerOsQuoted(`${comment} - block webproxy public`)}] destination=0`,
    `/ip firewall filter move [find where comment=${routerOsQuoted(`${comment} - allow webproxy local`)}] destination=0`,
    `/ip firewall filter move [find where comment=${proxyCommentText}] destination=0`,
    ...(httpsMode ? [
      `/ip firewall filter move [find where comment=${routerOsQuoted(`${proxyComment} HTTPS`)}] destination=0`
    ] : []),
    `/ip firewall filter move [find where comment=${routerOsQuoted(`${comment} - allow billing isolir`)}] destination=0`,
    '',
    `:put "Redirect isolir Billing selesai. Pastikan profile isolir memberi IP dalam subnet ${sourceSubnet}"`,
    `:put "Cek interface-list WAN: /interface list member print where list=${wanList}"`,
    ':put "Cek rule hit counter: /ip firewall nat print stats where comment~\\"Generated by Billing\\""'
  ];
    ...
    <div class="stack routeros-guide">
      ...
      <div class="form-grid routeros-guide-grid">
        ...
        <label class="field">
          ...
          <input name="wanInterface" value="${escapeHtml(defaults.wanInterface || '')}" placeholder="ether1 / pppoe-out1">
          <small class="muted">Opsional. Jika diisi, script otomatis memasukkan interface ini ke list WAN.</small>
        </label>
        <label class="field">
