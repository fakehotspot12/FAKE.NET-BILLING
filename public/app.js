...
async function ensureWebPushRegistration() {
  ...
  if (!webPushRegistration) {
    webPushRegistration = await navigator.serviceWorker.register('/service-worker.js?v=fakenet-billing-2.5.7', {
      scope: '/'
    });
  }
...
function currentBranding() {
  ...
  return {
    ...
    appVersion: state.branding.appVersion || state.settings.appInfo?.version || '2.5.7',
    buildVersion: state.branding.buildVersion || state.branding.appVersion || state.settings.appInfo?.version || '2.5.7',
